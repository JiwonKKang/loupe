...
function Onboarding(props) {
  const { Button, KeyHint } = window.LoupeDesignSystem_045e3b;
  const [step, setStep] = React.useState(0);
  const [repo, setRepo] = React.useState('monorepo / api');
  ...
  const canNext = step === 1 ? tested : true;
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
      ...
      <div style={{ width: 480, background: 'var(--surface-card)',
        ...
        {step === 0 && (
          <div>
            <button style={{ ...fieldStyle, height: 48, display: 'flex', alignItems: 'center',
              gap: 11, cursor: 'pointer', textAlign: 'left', color: 'var(--text-secondary)' }}>
              <span style={{ color: 'var(--accent)' }}><Ico d={folder} /></span>
              Browse for a folder…
            </button>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              ...
              <select value={base} onChange={(e) => setBase(e.target.value)} style={{ ...fieldStyle, fontFamily: 'var(--font-mono)' }}>
                <option>main</option><option>release/24.2</option><option>develop</option>
              </select>
            </div>
            <div>
              <label style={labelStyle}>Target</label>
              <select value={target} onChange={(e) => setTarget(e.target.value)} style={{ ...fieldStyle, fontFamily: 'var(--font-mono)' }}>
            ...
            <div style={{ font: 'var(--text-sm)/1.5 var(--font-ui)', color: 'var(--text-tertiary)' }}>
              8 changed functions across 3 chapters, ordered by dataflow.
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginTop: 30 }}>
          ...
          <div style={{ marginLeft: 'auto' }}>
            {step < 2
              ...
              : <Button variant="primary" onClick={() => props.onFinish()}>Start review</Button>}
          </div>
        </div>
      </div>
